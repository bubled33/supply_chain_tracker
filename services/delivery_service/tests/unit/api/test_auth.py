...
from src.api.deps.getters import get_courier_service, auth_provider

TEST_SECRET = "test-secret-for-auth-tests"
TEST_USERNAME = "testuser"

test_app = FastAPI()
test_app.include_router(courier_router)
...
def client():
    ...
    test_app.dependency_overrides[get_courier_service] = lambda: mock_service
...
def test_protected_route_with_valid_token(client):
    response = client.get("/couriers", headers={"Authorization": f"Bearer {_token()}"})
    assert response.status_code == 200


def test_protected_route_with_expired_token(client):
    response = client.get("/couriers", headers={"Authorization": f"Bearer {_expired_token()}"})
    assert response.status_code == 401
...
def test_protected_route_with_bad_signature(client):
    ...
    response = client.get("/couriers", headers={"Authorization": f"Bearer {bad_token}"})
    assert response.status_code == 401


def test_delete_courier_requires_admin(client):
    courier_id = uuid4()
...
def test_delete_courier_admin_succeeds(client):
    courier_id = uuid4()
    mock_service = MagicMock()
    mock_service.get = AsyncMock(return_value=MagicMock(courier_id=courier_id))
    mock_service.delete = AsyncMock(return_value=None)
    test_app.dependency_overrides[get_courier_service] = lambda: mock_service
